...
- `backups`：超过请求数量的合格备用结果。
- `rejected`：未通过结果及其 `failed_rules`。
...
安装第三方 Skill 前请先审查 `SKILL.md` 和脚本源码。Skill 会访问小红书页面并写入用户请求的本地报告，但不应读取或导出 Cookie、密码、浏览器配置文件、本地存储或其他认证材料。
